 ...
 */
package com.jeemicro.weixin.modules.cms.utils;
...
public class CmsUtils {
    ...
    public static String getUrlDynamic(Article article) {
        ...
        /*String ss = article.getCategory().getId();
        System.out.println(article.getCategory().getId());*/
        if(article.getCategory()==null||"".equals(article.getCategory())){
        	Category category = new Category(article.getCategoryId());
        	article.setCategory(category);
        }
        if(article.getCategory().getId().equals("13")){
        	str.append("/viewBm-").append(article.getCategory().getId()).append("-").append(article.getId()).append(Global.getUrlSuffix());
        }else{
